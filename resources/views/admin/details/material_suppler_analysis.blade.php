...
<div class="row">
	...
	<div class="col-xl-12 col-xxl-12 col-lg-12">
		<div class="card">
		    ...
		    <div class="row card-title-align">
				<div class="col-lg-8 col-xl-8 col-xxl-8">
					...
					<div class="mr-auto pr-3">
						<h4 class="text-black fs-20">Material Supplier Analysis</h4>
					</div>
					</div>
				</div>
			...
			<form method="post" action="{{ url('material-supplier-analysis')}}">
			@csrf
			<div class="row mt-1 pt-2 pb-2 filter-box">
				<label class="col-lg-2 col-xl-2 col-xxl-2 col-form-label text-right">Select Supplier</label>
					...
					<table border=0 style="border:none;width:100%;font-weight:500;">
						...
						<table class="display" cellspacing=0 cellpadding=0 border=0 style="border:none;width:100%;font-size:13px;font-weight:500;">
							<thead>
							  ...
							  <tr>
								@if(!empty($mdata))
									<td class="st-td" colspan=10 align="left" style="font-size:14px;">
										<label style="padding-top:10px;">Supplier:&nbsp;&nbsp;&nbsp;<b>{{$mdata[0]->supplier_name??""}}</b></label>
										@if($sdate!="" and $edate!="")
								...
								<tr style="line-height:30px;">
									...
									<th class="st-td">PROJECT_NAME</th>
									<th class="st-td">INVOICE_NO</th>
									<th class="st-td">INVOICE_DATE</th>
									<th class="st-td">ITEM/DESCRIPTION</th>
									<th class="st-td">QUANTITY</th>
									<th class="st-td">UNIT</th>
									<th class="st-td">RATE</th>
									<th class="st-td">GST.AMT</th>
									<th class="st-td">TOTAL.AMT</th>
								</tr>
							</thead>
							<tbody>
							
							@php $gst_tot=0; $tot_amt=0; @endphp							
							
							@if(!empty($mdata))
							   @foreach($mdata as $key=>$r)
						   
								@php
								$gst_tot+=$r->gst_amount;
								$tot_amt+=$r->amount;
								@endphp
								...
								<tr style="height:25px;">
									...
									<td class="st-td">{{date_create($r->supply_date)->format('d-m-Y')}}</td>
									<td class="st-td">{{$r->material_type_name}}( {{$r->sub_type_name}} )</td>
									<td class="st-td">{{$r->quantity}}</td>
									<td class="st-td">{{$r->material_unit}}</td>
									<td class="st-td" align="right">{{number_format($r->price,2,'.','')}}</td>
									<td class="st-td" align="right">{{number_format($r->gst_amount,2,'.','')}}</td>
							...
							<tr style="font-size:15px;font-weight:600;height:25px;">
									<td colspan=8 class="st-td" align="right">Total</td>
									<td class="st-td" align="right">{{number_format($gst_tot,2,'.','')}}</td>
