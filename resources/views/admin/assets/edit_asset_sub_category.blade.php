<form method="POST" action="{{url('update-asset-sub-category')}}" enctype="multipart/form-data">
@csrf

<input type="hidden"  name="ed_sub_category_id" value="{{$sc->id}}">

<div class="modal-body" style="padding-bottom:.5rem;">
	<div class="form-group">
	<label>Category</label>
	   <select class="form-control input-default"  name="ed_category_id" required>
	   <option value="">--select--</option>
	   @foreach($cats as $r)
	   <option value="{{$r->id}}" @if($r->id=$sc->asset_category_id){{__('selected')}}@endif >{{$r->category_name}}</option>
	   @endforeach
	   </select>
	</div>
